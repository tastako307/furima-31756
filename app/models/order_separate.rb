class OrderSeparate
  ...
  validates :post_number, format: { with: /\A\d{3}[-]\d{4}\z/, message: '例：123-3456 郵便番号は左のように入力してください。なおこの項目の入力は全て半角で入力を行います'}
  ...
  validates :phone_number, format: { with: /\A0[0-9]{9,10}\z/, message: '電話番号は、0から始まる10桁、または11桁の半角数字で入力してください'}
